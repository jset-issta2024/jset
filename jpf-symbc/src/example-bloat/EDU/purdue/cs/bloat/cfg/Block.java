 ...
 * graphs. A basic block is always entered at its beginning and exits at
 * its end.  That is, its first statement is a label and its last
 * statement is a jump.  There are no other labels or jumps in between.
 * <p>
 * Each <tt>Block</tt> knows its parent block and its children in the
 * dominator and postdominator trees.  It also knows which blocks are in
 * its dominance frontier and its postdominance frontier.
 *
 * @see FlowGraph
 * @see DominatorTree
...
public class Block extends GraphNode
{
   ...
   *        of code at which the block begins.
   * @param graph
  ...
  Block(Label label, FlowGraph graph) {
    ...
    stackOptimizer = new StackOptimizer(this); //make StackOptimizer object
   ...
   * </ul>
   *
   * A <i>loop</i> is a strongly connected component of a control flow 
   * graph.  A loop's <i>header</i> is the block that dominates all 
   * other blocks in the loop.  A loop is <i>reducible</i> if the only
   ...
   * That is, it returns this Block's children in the dominator tree for
   * the CFG.
   */  
  ...
  /**
   * Returns the immediate dominator of this Block.  That is, it returns
   * the Block's parent in the dominator tree, its immediate dominator.
   */
  Block domParent()
  {
    return domParent;
  ...
  /** 
   * Specifies that Block dominates this Block (parent in the
   * dominator tree, the immediate dominator).
   *
   * @param block
   ...
   * dominates a node Y when every path from the first node in the CFG
   * (Enter) to Y must pass through X.
   */
  ...
  /**
   * Determines whether or not this block postdominates a given block.  A
   * block X is said to postdominate a block Y when every path from Y to
   * the last node in the CFG (Exit) passes through X.  This relationship
   * can be thought of as the reverse of dominance.  That is, X dominates
   * Y in the reverse CFG.
   *
   * @see DominatorTree
   */  
   ...
   * that X dominates a predacessor of Y, but does not strictly dominate
   * Y.  Nodes in the dominance frontier always have more than one parent
   * (a join).
   *
   * @see DominanceFrontier
   */  
  ...
  /**
   * Returns the postdominance frontier for this node.  A postdominace 
   * frontier is essentially the same as a dominace frontier, but the
   * postdominance relationship is used instead of the dominance 
   * relationship.
   */
