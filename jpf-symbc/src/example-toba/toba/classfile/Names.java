...
public static int hashinterface(String name, String sig)
{
    ...
    for (i = 0; name != null && i < name.length(); i++) {
	c = name.charAt(i);
	sum = HashMultName * sum + (int)c;
    }
    for (i = 0; sig != null && i < sig.length(); i++) {
	c = sig.charAt(i);
    ...
    (String name, String classnm, String sig, StringBuffer b)
{
    ...
    for (i = 0; classnm != null && i < classnm.length(); i++) {
	c = classnm.charAt(i);
	if (c == '.')
...
{
	if (name == null || name.length() == 0) return false; //added by zhenbang to avoid stringindexoutofboundexception
