...
Field(ClassFile d, Constant c[])
    throws IOException
{
    access = d.readUnsignedShort();
    int j = d.readUnsignedShort();
    if (j >= 0 && j < c.length && c[j] != null && c[j].value instanceof String)				//bug fixed by zhenbang
    	name = (String)c[j].value;
    else
    	name = null;
    j = d.readUnsignedShort();
    if (j >= 0 && j < c.length && c[j] != null && c[j].value instanceof String)
    	signature = (String)c[j].value;
    else
    	signature = null;
    attributes = new Attribute[d.readUnsignedShort()];
    //System.out.println("atr length is : " + attributes.length);
