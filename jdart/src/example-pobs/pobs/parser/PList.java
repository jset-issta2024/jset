 ...
 * The user is responsible for making sure the item parser does not swollow
...
public class PList extends pobs.PParser {
     ...
     * Current implementation constructs a POBS parser, future implementations
     * may use dedicated code for better performance.
     * @param item		parser which matches the items in the list
     * @param delimiter parser which matches the delimiters interleaving the
     * 					items
     * @param min		minimum number of items required
     * @param max		maximum number of items parsed
     */
    public PList(
        pobs.PObject item,
        pobs.PObject delimiter,
        long min,
        long max) {
        ...
        if (min > 1) {
            this.parser =
                new pobs.parser.PSequence(
                    item,
                    new pobs.parser.PRepeat(
                        new pobs.parser.PSequence(delimiter, item),
                        min - 1,
                        max));
        } else {
            this.parser =
                new pobs.parser.POptional(
                    new pobs.parser.PSequence(
                        item,
                        new pobs.parser.PRepeat(
                            new pobs.parser.PSequence(delimiter, item),
    ...
    public pobs.PMatch parse(
       ...
       //System.out.println("Enterint Plist Parsing!");
