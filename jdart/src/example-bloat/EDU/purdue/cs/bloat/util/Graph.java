 ...
 * GraphNodes are created and are added to the Graph before the edges 
 * can be constructed.  Each GraphNode as a unique key associated with 
 * it.  For instance, if a Graph represents a control flow graph, each
...
public class Graph
{
  ...
  private Collection roots;     // The root nodes of this Graph (no predacessors)
  private Collection revRoots;  // Reverse root nodes of this Graph (no successors)

  // These counts are used to determine when certain actions (such as updating the
  ...
  /**
   * Compile a collection of root nodes in this Graph.  If reverse is
   * true, the collection will contain those nodes with no predacessor
   * nodes. If reverse is false, the collection will contain those
   * nodes with no sucessor nodes.
   *
   * @param c
  ...
  {
    HashSet visited = new HashSet(nodes.size() * 2);
  ...
  /**
   * Determines whether or not a node v is an ancestor (has a lower
   * pre-order index and a higher post-order index) of a node w.
   *
   * @param v
  ...
  /**
   * Returns the index of a given node in a pre-order ordering of this 
   * Graph.
   */
  ...
  /**
   * Returns the index of a given node in a post-order ordering of 
   * this Graph.
   */
  ...
  private void buildLists()
  {
    ...
    while (iter.hasNext()) {
      ...
      Assert.isTrue(nodes.containsValue(root),
		    "Graph does not contain " + root);
   ...
   *
   * @param key
   *        A unique value associated with this node.  For instance,
   *        if this Graph represented a control flow graph, the key
   *        would be a basic block.
   * @param node
  ...
  /**
   * Returns a Set of the keys used to uniquely identify the nodes in
   * this Graph.
   */
  public Set keySet()
  {
  ...
  /**
   * A NodeMap that stores nodes.  I guess we use this data structure 
   * to make it easier to ensure that there are not duplicate nodes in
   * the Graph.  A HashMap is used as the underlying stored mechanism.
   */
  ...
  /**
   * NodeList represents a list of nodes.  Special provisions must be
   * made for methods such as indexOf() and iterator().  A NodeList is
   * used to store the pre-order and post-order travsersals of the
   * Graph.
   */
  ...
  class EdgeSet extends AbstractSet
  {
    ...
    /**
     * Removes all of the nodes in this set that are specified in a 
     * given Collection.
     *
     * @param c
     *        The nodes to remove.
      ...
      return new Iterator()
	{
	  ...
	  public Object next()
		...
		throw new ConcurrentModificationException();
	      ...
	      Assert.isTrue(nodes.containsValue(last),
			    last + " not found in graph");
	      Assert.isTrue(nodes.containsValue(node),
