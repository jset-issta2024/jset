...
//        System.out.println( buffer.toString());
        ...
        if(SnakeyamlConfig.SYMB_FLAG){
			char[] data;
			if(SnakeyamlConfig.TOKEN_SYMB){
...
//                        data[i]=Debug.makeConcolicChar("sym_cnf_char"+i, ""+(int)data[i]);
    ...
    private void update() {
        try {
            ...
            System.out.println("update read: "+read+"   update buffer_size:"+BUFFER_SIZE);
//            for(int i=0;i<buffer.length;i++){ 
//    			if(((char)buffer[i])!=' '){ 
//    				buffer[i]=Debug.makeConcolicChar("sym_cnf_char"+i, ""+(int)buffer[i]); 
//    			} 
//    		}
            
//            String s=new String(buffer);
//            if(SnakeyamlConfig.SYMB_FLAG && SnakeyamlConfig.TOKEN_SYMB){
//            	Debug.setInput(s, "3");
