...
/**
 * <tt>Instruction</tt> represents a single instruction in the JVM.  
 * All <tt>Instruction</tt>s known their opcode.  Some instructions have
 * an operand.  Operands are integers, floats, or one of the special
 * classes that represent multiple operands such as <tt>IncOperand</tt>
 * and <tt>Switch</tt>.
 *
 * @see IncOperand
 * @see Switch
...
public class Instruction implements Opcode
{
  ...
  public Instruction(int opcode, Object operand)
  {
    ...
    Assert.isTrue(opcode == opcXMap[opcode],
   ...
   */
  public Instruction(byte[] code, int index, int[] targets, int[] lookups,
		     LocalVariable[] locals, ConstantPool constants)
  {
    int i, j;
    int incr;
    int dim;
    ...
    case opc_bipush:
      ...
      break;
    case opc_sipush:
      operand = new Integer(toShort(code[index+1], code[index+2]));
      break;
    case opc_ldc:
    ...
    case opc_aload:
      i = toUByte(code[index+1]);
      operand = i < locals.length && locals[i] != null
	? locals[i] : new LocalVariable(i);
      break;
    case opc_iload_0:
    case opc_lload_0:
    case opc_fload_0:
    case opc_dload_0:
    case opc_aload_0:
      operand = 0 < locals.length && locals[0] != null
	? locals[0] : new LocalVariable(0);
      break;
    case opc_iload_1:
    case opc_lload_1:
    case opc_fload_1:
    case opc_dload_1:
    case opc_aload_1:
      operand = 1 < locals.length && locals[1] != null
	? locals[1] : new LocalVariable(1);
      break;
    case opc_iload_2:
    case opc_lload_2:
    case opc_fload_2:
    case opc_dload_2:
    case opc_aload_2:
      operand = 2 < locals.length && locals[2] != null
	? locals[2] : new LocalVariable(2);
      break;
    case opc_iload_3:
    case opc_lload_3:
    case opc_fload_3:
    case opc_dload_3:
    case opc_aload_3:
      operand = 3 < locals.length && locals[3] != null
	? locals[3] : new LocalVariable(3);
      break;
    case opc_istore:
    case opc_lstore:
    case opc_fstore:
    ...
    case opc_iinc:
      i = toUByte(code[index+1]);
      incr = code[index+2];
      operand = new IncOperand(i < locals.length && locals[i] != null
			       ? locals[i] : new LocalVariable(i), incr);
      break;
    case opc_ifeq:
    case opc_ifne:
    case opc_iflt:
    ...
    case opc_if_acmpeq:
    case opc_if_acmpne:
      Assert.isTrue(targets.length == 1,
		    "Illegal instruction: " + opcNames[opc]);
    ...
    case opc_multianewarray:
      i = toUShort(code[index+1], code[index+2]);
      dim = toUByte(code[index+3]);
      operand = new MultiArrayOperand(
				      (Type) constants.constantAt(i), dim);
      break;
    case opc_rc:
      i = toUByte(code[index+1]);
  ...
  /** 
   * Returns the original (non-mapped) opcode used to create this
   * Instruction.  
   */
  ...
  /** 
   * Sets a flag that determines whether or not the "slow" version of
   * the instruction should be generated.  For example, if useSlow is
   * true, "iload 2" is generated instead of "iload_2".  
   */
   ...
   *
   * @return
   *        true if the instruction is an increment,
   *        false if not.
   */
   ...
   *
   * @return
   *        true if the instruction is a throw,
   *        false if not.
   */
   ...
   *
   * @return
   *        true if the instruction is a ret,
   *        false if not.
   */
  ...
  {
    return (toUByte(b1) << 24) | (toUByte(b2) << 16) |
      (toUByte(b3) << 8) | toUByte(b4);
  }
  ...
  /**
   * Returns the category of this instruction.  An instruction's
   * category is basically the width of the value the instruction
   * places on the stack.  Types <tt>long</tt> and <tt>double</tt> are
   * Category 2.  All other types are Category 1.
   */
  public int category() {
    switch(this.opcode) {
    case opcx_lload:
