...
public class EndTagTypeGenericImplementation extends EndTagType {
	...
	protected Tag constructTagAt(final Source source, final int pos) {
		...
		//System.out.println("lll"+pos+"lll2"+nameBegin);
		String name=null;
		...
		if (isStatic()) {
			...
			if (!parseText.containsAt(getClosingDelimiter(),startDelimiterEnd)) {
				...
				if (source.logger != null && source.logger.isErrorEnabled()) source.logger.error(source.getRowColumnVector(pos).appendTo(new StringBuilder(200).append("EndTag of expected format ").append(staticString).append(" at ")).append(" not recognised as type '").append(getDescription()).append("' because it is missing the closing delimiter").toString());
				return null;
			}
			...
			//System.out.println("show name:"+name+"nameEnd"+nameEnd);
			int expectedClosingDelimiterPos=nameEnd;
			while (Segment.isWhiteSpace(parseText.charAt(expectedClosingDelimiterPos))) expectedClosingDelimiterPos++; // isWiteSpace can generate constraints!
			if (!parseText.containsAt(getClosingDelimiter(),expectedClosingDelimiterPos)) { // containsAt can generate constraints!
				...
				if (source.logger != null && source.logger.isErrorEnabled()) source.logger.error(source.getRowColumnVector(pos).appendTo(new StringBuilder(200).append("EndTag ").append(name).append(" at ")).append(" not recognised as type '").append(getDescription()).append("' because its name and closing delimiter are separated by characters other than white space").toString());
				return null;
			}
