...
public abstract class Tag extends Segment {
	...
	static final Tag[] parseAll(final Source source, final boolean assumeNoNestedTags) {
    	...
    	if (source.end!=0) {
			final ParseText parseText=source.getParseText();
			Tag tag=parseAllgetNextTag(source,parseText,0,assumeNoNestedTags); //generate constraints!
			while (tag!=null) {
				list.add(tag);
				...
				tag=parseAllgetNextTag(source,parseText,pos,assumeNoNestedTags); //generate constraints!
			}
		}
	...
	private static final Tag parseAllgetNextTag(final Source source, final ParseText parseText, final int pos, final boolean assumeNoNestedTags) {
		try {
			int begin=pos;
			do {
				begin=parseText.indexOf('<',begin); // this assumes that all tags start with '<' // can generate constraints!
				if (begin==-1) return null;
				final Tag tag=TagType.getTagAt(source,begin,false,assumeNoNestedTags); // can generate constraints
				if (tag!=null) {
