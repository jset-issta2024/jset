...
//		//consumeWhitespace();
//		System.out.println("start to create: "+tokenCharPos+" : "+token);
		if(NanojsonConfig.SYMB_FLAG==false || NanojsonConfig.TOKEN_SYMB==false){
			return token;
		}
		...
		int a;
		if (oldtoken == token && oldpos == tokenCharPos) {
//			a = Debug.makeConcolicInteger("sym_token_" + (token_index - 1),
//					"" + (int) token, "1");
		} else {
			//System.out.println("start to create: " + pos + ":" + token);
//			a = Debug.makeConcolicInteger("sym_token_" + token_index, ""
//					+ (int) token, "1");
			token_index++;
			oldtoken = token;
			oldpos = tokenCharPos;
