...
/**
 * Tree represents the expression tree of a basic Block.  It consists 
 * of an operand (expression) stack comprised of expressions and a
...
public class Tree extends Node implements InstructionVisitor, Opcode
{
   ...
   * @param predStack
   *        The contents of the operand stack from the previous basic Block.
   */
  ...
  /**
   * Add a Collection of local variables to the block.  Add an InitStmt 
   ...
   * overridden because some adjustments may need to be made to the
   * Nodes in the tree when certain operations are performed.
   */
  ...
  /**
   * When we build the expression tree, there may be items left over
   * on the operand stack.  We want to save these items.  If USE_STACK
   * is true, then we place these items into stack variables.  If
   * USE_STACK is false, then we place the items into local variables.
   */  
  private void saveStack() {
	...
	if (! (expr instanceof StackExpr) ||
	  ...
	  Stmt store = new ExprStmt(new StoreExpr(target,
						  expr, expr.type()));
  ...
  /**
   * Add a statement to this Tree statement list and specify that this
   * is the statement's parent.
   */
   ...
   * It is assumed that the last statement in the statement list is a 
   * jump statement.
   *
   * @see JumpStmt
   */
  ...
  {
    Stmt last = lastStmt();
    Assert.isTrue(last instanceof JumpStmt,
		  "Last statement of " + block + " is " + last + 
		  ", not a jump");
  ...
  {
    MethodEditor method = block.graph().method();
  ...
  /**
   * The last instruction we saw.  addInst(Instruction) needs this
   * information.
   */
  ...
  /**
   * Adds an instruction that does not change the control flow
  ...
  /** 
   * Add an instruction such as <i>ret</i> or <i>astore</i> that may
   * involve a subroutine.
   *
   * @param inst
   *        Instruction to add.
   * @param sub
  ...
  {
    Assert.isTrue(inst.isRet() || inst.opcodeClass() == opcx_astore,
		  "Wrong addInstruction called with " + inst);
  ...
  /**
   * Add a label to the statement list.  A label is inserted before a
   * dup statement, but after any other statement.
   *
   * @param label
   *        Label to add.
   */
  ...
  /**
   * Private method that adds an instruction to the Tree.  The visit
   * method of the instruction is called with this tree as the visitor.
   * The instruction, in turn, calls the appropriate method of this
   * for adding the instruction to the statement list.
   *
   * @param inst
   *        The <tt>Instruction</tt> to add to the <tt>Tree</tt>
   * @param saveValue
   *        Do we save expressions on the operand stack to stack 
  ...
  /**
   * Private method that adds an instruction to the tree.  Before
   * dispatching the addition off to addInst(Instruction, boolean),
   * it optimizes(?) dup instructions as outlined below:
   */  
  // (S0, S1) := dup(X)
  // L := S1                             --> use (L := X)
  // use S0
   ...
   * They are created when the USE_STACK flag is not set.
   *
   * @param index
  ...
  /** 
   * Returns a local variable (<tt>LocalExpr</tt>) located in this
   * method.
   *
   * @param type
  ...
  {
    LocalVariable var = block.graph().method().newLocal(type);
  ...
  {
    Expr index = (Expr) stack.pop(Type.INTEGER);
    Expr array = (Expr) stack.pop(Type.INTEGER.arrayType());
  ...
  {
    Expr index = (Expr) stack.pop(Type.INTEGER);
    Expr array = (Expr) stack.pop(Type.LONG.arrayType());
  ...
  {
    Expr index = (Expr) stack.pop(Type.INTEGER);
    Expr array = (Expr) stack.pop(Type.FLOAT.arrayType());
  ...
  {
    Expr index = (Expr) stack.pop(Type.INTEGER);
    Expr array = (Expr) stack.pop(Type.DOUBLE.arrayType());
  ...
  {
    Expr index = (Expr) stack.pop(Type.INTEGER);
    Expr array = (Expr) stack.pop(Type.OBJECT.arrayType());
    Expr top = new ArrayRefExpr(array, index,
  ...
  {
    Expr index = (Expr) stack.pop(Type.INTEGER);
    Expr array = (Expr) stack.pop(Type.BYTE.arrayType());
  ...
  {
    Expr index = (Expr) stack.pop(Type.INTEGER);
    Expr array = (Expr) stack.pop(Type.CHARACTER.arrayType());
    Expr top = new ArrayRefExpr(array, index,
  ...
  {
    Expr index = (Expr) stack.pop(Type.INTEGER);
    Expr array = (Expr) stack.pop(Type.SHORT.arrayType());
    Expr top = new ArrayRefExpr(array, index, Type.SHORT, Type.SHORT);
    stack.push(top);
  }
  
  /**
   * Deals with an expression that stores a value.  It either pushes it
   * on the operand stack or adds a statement depending on the value of
   * saveValue.
   *
   * @param target
  ...
  {
    LocalVariable operand = (LocalVariable) inst.operand();
    Expr expr = (Expr) stack.pop(Type.INTEGER);
   ...
   * instruction is an address add an AddressStoreStmt to the tree, else
   * add a StoreStmt to the tree consisting of a LocalExpr and the top
   * Expr on the operand stack.
   *
  ...
  public void visit_astore(Instruction inst) {
    ...
    if (expr.type().isAddress()) {
      ...
      expr = (Expr) stack.pop(Type.ADDRESS);
  ...
  {
    Expr value = (Expr) stack.pop(Type.INTEGER);
    Expr index = (Expr) stack.pop(Type.INTEGER);
    Expr array = (Expr) stack.pop(Type.INTEGER.arrayType());
    ArrayRefExpr target = new ArrayRefExpr(array, index,
  ...
  {
    Expr value = (Expr) stack.pop(Type.CHARACTER);
    Expr index = (Expr) stack.pop(Type.INTEGER);
    Expr array = (Expr) stack.pop(Type.CHARACTER.arrayType());
    ArrayRefExpr target = new ArrayRefExpr(array, index,
					   Type.CHARACTER, Type.CHARACTER);
  ...
  /**
   * Pop the expression off the top of the stack and add it as an
   * ExprStmt to the statement list.
   *
   * @see ExprStmt
   */
  ...
  /**
   * When processing the <i>dup</i> instructions one of two situations
   * can occur.  If the <tt>USE_STACK</tt> flag is set, then a 
   * StackManipStmt is created to represent the transformation that the
   * <i>dup</i> instruction performs on the stack.  If the 
   * <tt>USE_STACK</tt> flag is not set, then the transformation is
   * simulated by creating new local variables containing the 
   * appropriate element of the stack.
   *
   * @see LocalExpr
   * @see StackExpr
   * @see StackManipStmt
  ...
  /**
   * Produces a StackManipStmt that represents how the stack is changed
   * as a result of a dup instruction.  It should only be used when 
   * USE_STACK is true.  
   * <p>
   * dup instructions change the top n elements of the JVM stack.  This
   * method takes the original top n elements of the stack, an integer
   * array representing the transformation (for instance, if s[0] = 1,
   * then the top element of the new stack should contain the 
   * second-from-the-top element of the old stack), and integer 
   * representing the dup instruction.
   *
   * @param source
  ...
  {
    Expr right = (Expr) stack.pop(Type.INTEGER);
    Expr left = (Expr) stack.pop(Type.INTEGER);
    Expr top = new ArithExpr(ArithExpr.ADD, left, right, Type.INTEGER);
    stack.push(top);
  }
  ...
  public void visit_isub(Instruction inst)
  {
    Expr right = (Expr) stack.pop(Type.INTEGER);
    Expr left = (Expr) stack.pop(Type.INTEGER);
    Expr top = new ArithExpr(ArithExpr.SUB, left, right, Type.INTEGER);
    stack.push(top);
  }
  ...
  public void visit_imul(Instruction inst)
  {
    Expr right = (Expr) stack.pop(Type.INTEGER);
    Expr left = (Expr) stack.pop(Type.INTEGER);
    Expr top = new ArithExpr(ArithExpr.MUL, left, right, Type.INTEGER);
    stack.push(top);
  }
  ...
  public void visit_idiv(Instruction inst)
  {
    ...
    Expr top = new ArithExpr(ArithExpr.DIV, left, check, Type.INTEGER);
    stack.push(top);
  }
  ...
  public void visit_irem(Instruction inst)
  {
    ...
    Expr left = (Expr) stack.pop(Type.INTEGER);
    Expr check = new ZeroCheckExpr(right, Type.INTEGER);
    Expr top = new ArithExpr(ArithExpr.REM, left, check, Type.INTEGER);
    stack.push(top);
  }
  ...
  public void visit_ishl(Instruction inst)
  {
    Expr right = (Expr) stack.pop(Type.INTEGER);
    Expr left = (Expr) stack.pop(Type.INTEGER);
    Expr top = new ShiftExpr(ShiftExpr.LEFT, left, right, Type.INTEGER);
    stack.push(top);
  }
  ...
  public void visit_ishr(Instruction inst)
  {
    Expr right = (Expr) stack.pop(Type.INTEGER);
    Expr left = (Expr) stack.pop(Type.INTEGER);
    Expr top = new ShiftExpr(ShiftExpr.RIGHT, left, right, Type.INTEGER);
    stack.push(top);
  }
  ...
  public void visit_iand(Instruction inst)
  {
    Expr right = (Expr) stack.pop(Type.INTEGER);
    Expr left = (Expr) stack.pop(Type.INTEGER);
    Expr top = new ArithExpr(ArithExpr.AND, left, right, Type.INTEGER);
    stack.push(top);
  }
  ...
  public void visit_ior(Instruction inst)
  {
    Expr right = (Expr) stack.pop(Type.INTEGER);
    Expr left = (Expr) stack.pop(Type.INTEGER);
    Expr top = new ArithExpr(ArithExpr.IOR, left, right, Type.INTEGER);
    stack.push(top);
  }
  ...
  public void visit_ixor(Instruction inst)
  {
    Expr right = (Expr) stack.pop(Type.INTEGER);
    Expr left = (Expr) stack.pop(Type.INTEGER);
    Expr top = new ArithExpr(ArithExpr.XOR, left, right, Type.INTEGER);
    stack.push(top);
  }
  ...
  public void visit_iinc(Instruction inst)
  {
    ...
    if (incr < 0) {
      Expr right = new ConstantExpr(new Integer(-incr), Type.INTEGER);
      Expr left = new LocalExpr(operand.var().index(), Type.INTEGER);
  ...
  public void visit_lcmp(Instruction inst)
  {
    Expr right = (Expr) stack.pop(Type.LONG);
    Expr left = (Expr) stack.pop(Type.LONG);
    Expr top = new ArithExpr(ArithExpr.CMP, left, right, Type.INTEGER);
    stack.push(top);
  }
  ...
  public void visit_fcmpl(Instruction inst)
  {
    Expr right = (Expr) stack.pop(Type.FLOAT);
    Expr left = (Expr) stack.pop(Type.FLOAT);
    Expr top = new ArithExpr(ArithExpr.CMPL, left, right, Type.INTEGER);
    stack.push(top);
  }
  ...
  public void visit_fcmpg(Instruction inst)
  {
    Expr right = (Expr) stack.pop(Type.FLOAT);
    Expr left = (Expr) stack.pop(Type.FLOAT);
    Expr top = new ArithExpr(ArithExpr.CMPG, left, right, Type.INTEGER);
    stack.push(top);
  }
  ...
  public void visit_ifeq(Instruction inst)
  {
    ...
    Expr left = stack.pop(Type.INTEGER);
    Block t = (Block) block.graph().getNode(inst.operand());
  ...
  public void visit_if_icmpeq(Instruction inst)
  {
    ...
    Expr right = stack.pop(Type.INTEGER);
    Expr left = stack.pop(Type.INTEGER);
    Block t = (Block) block.graph().getNode(inst.operand());
    Assert.isTrue(t != null, "No block for " + inst);
  ...
  public void visit_jsr(Instruction inst)
  {
    ...
    // so we can't save it.
    Subroutine sub = block.graph().labelSub((Label) inst.operand());
  ...
  public void visit_switch(Instruction inst)
  {
    ...
    Expr index = stack.pop(Type.INTEGER);

    Switch sw = (Switch) inst.operand();

    Block defaultTarget = (Block) block.graph().getNode(sw.defaultTarget());
  ...
  public void visit_getstatic(Instruction inst)
  {
	...
	block.graph().method().declaringClass().context();
  ...
  /** 
   * Creates either a CallMethodExpr or a CallStaticExpr to represent
   * a method call.  After obtaining some information about the
   * method. The parameters to the methods are popped from the operand
   * stack.  If the method is not static, the "this" object is popped
   * from the operand stack.  A CallMethodExpr is created for a 
   * non-static method and a CallStaticExpr is created for a static
   * method.  If the method returns a value, the Call*Expr is pushed
   * onto the stack.  If the method has no return value, it is wrapped
   * in an ExprStmt and is added to the statement list.
   */
  private void addCall(Instruction inst, int kind) {
    ...
    if (inst.opcodeClass() != opcx_invokestatic) {
      Expr obj = stack.pop(Type.OBJECT);
  ...
  public void visit_newarray(Instruction inst) {
    Type type = (Type) inst.operand();
    Expr size = stack.pop(Type.INTEGER);
  ...
  {
    Expr expr = (Expr) stack.pop(Type.THROWABLE);
  ...
  public void visit_multianewarray(Instruction inst)
  {
    ...
    Expr top = new NewMultiArrayExpr(dim,
				     type.elementType(dim.length), type);
  ...
  public void visit_aupdate(Instruction inst)
  {
    ...
    Expr object = stack.peek(depth.intValue());
    stack.replace(depth.intValue(),
		  new UCExpr(object, UCExpr.POINTER, object.type()));
  }
  ...
  public void visit_supdate(Instruction inst)
  {
    ...
    Expr object = stack.peek(depth.intValue());
    stack.replace(depth.intValue(),
		  new UCExpr(object, UCExpr.SCALAR, object.type()));
  }
  ...
  {
    Expr end = (Expr) stack.pop(Type.INTEGER);
    Expr start = (Expr) stack.pop(Type.INTEGER);
    Expr array = (Expr) stack.pop(Type.OBJECT.arrayType());
