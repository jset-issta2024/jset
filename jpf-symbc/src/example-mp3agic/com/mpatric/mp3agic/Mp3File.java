...
//		int bitrateSym = (byte)Debug.makeConcolicChar("sym_char_", ((int)bitrate)+"");
		int bitrateSym = (byte) Debug.makeSymbolicChar(((int)bitrate)+"");
		System.out.println("sym_char_" + ":" + (int)bitrateSym);
		
		MutableInteger count = bitrates.get(bitrateSym);
		if (count != null) {
			count.increment();
		} else {
			bitrates.put(bitrateSym, new MutableInteger(1));
		}
