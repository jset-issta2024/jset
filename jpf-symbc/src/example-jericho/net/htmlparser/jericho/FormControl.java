	...
	static FormControl construct(final Element element) {
		...
		if (tagName==HTMLElementName.INPUT) {
			...
			if (formControlType==null) {
				...
				if (element.source.logger != null && element.source.logger.isErrorEnabled()) element.source.logger.error(element.source.getRowColumnVector(element.begin).appendTo(new StringBuilder(200)).append(": INPUT control with unrecognised type \"").append(typeAttributeValue).append("\" assumed to be type \"text\"").toString());
