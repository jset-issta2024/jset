 ...
 * </pre>
 *
 * NOTE: The type for all occurrences of an inserted variable is the same as
 * the type of the first occurrence of the expression the variable replaces.
 * This type is guaranteed since we only group expression with equal types.
 */

/*
 * Okay, you asked for it: SSAPRE.  SSAPRE stands for Partial Redundency
 * Elimination in Static Single-Assignment form.  Partial Redundency
 * Elimination invovles finding multiple occurreces of the same expressions
 * (i.e. expressions that are lexically equivalent) and moving those
 * occurrences up higher in the CFG so that the expression is evaluated
 * fewer times.  Occurrences of the expression that has been moved are 
 * replaced with a temporary variable that is assigned the value of the
 * moved expression.  PRE is a superset of Common Subexpression Elimination.
 *
 ...
 *
 * PRE techniques have been around for quite sometime, but most algorithms
 * used a bit vector notation to represent occurrences of an expression.  
 * In order to apply those PRE techniques, the CFG in SSA form would have 
 * to be transformed into bit vector notation before PRE could be performed.
 * The modified bit vectors would then have to be transformed back into a
 * CFG in SSA form before any other optimizations (or code generation) could
 * take place.
 *
 * That was the way life was until [Chow, et. al.] came along.  At the 1997
 * PLDI conference, they presented a paper that showed how PRE could be
 * performed directly on a CFG in SSA form.  The paper itself is a rather
 * difficult read.  So don't feel bad if you don't understand it on the
 * first (or seventeenth) read.
 *
 * BLOAT performs SSAPRE on the CFG for optimizable methods.  Due to some
 * of Java's quirks (e.g. exceptions), this implementation does not follow
 * Chow's exactly.  However, it is very close and understanding this 
 * implementation will aid the understanding of Chow and vice versa.
 *
 * The first step in SSAPRE is to create a worklist of all expressions
 * that are candidates for SSAPRE.  Only first-order expressions are
 * entered into the worklist.  First-order expressions consist of a
 * single operators and two operands that are either a variable or
 * have a constant value.  For instance, a+b is a first-order
 * expression, while a+b-c is not.  The first-order expressions that
 * are inserted into the worklist are "real occurrences" of the
 * expression (as opposed to PHI-statements or PHI-operands, as we
 * shall shortly see).  In addition to real occurrences of the
 * expression, the worklist also contains kill statements.  Certain
 * constructs in Java, such as exceptions, method calls, and
 * synchronized blocks, limit the assumptions one can make about the
 * behavior of the code.  Thus, at places in the code where such
 * constructs occur, kills are inserted to say "We cannot move code
 * across this point."  The above is performed in collectOccurrences().
 * 
 * SSAPRE is then performed on each expression in the worklist.  The
 * first step in SSAPRE is to place PHI-functions.  PHI-functions are
 * similar to the phi-functions that occur in converting a CFG to SSA
 * form.  Essentially, a PHI-function is placed at a block (node in
 * the CFG) at which a join occurs (i.a. there are two paths that can
 * enter the block), at which the expression is available on at least
 * one of the incoming paths, and at which the expression will be used
 * again (e.g. we don't bother putting a PHI-function at the "exit"
 * node).  This leads us to place PHI-functions in two situations.
 * First, for each block that contains a real occurrence of the
 * expression, a PHI-function is placed at every block in its iterated
 * dominance frontier (DF+).  Second, a PHI-function is placed in
 * every block that contains a phi-function for either of the operands
 * to the expression.  At this point, the operands to the
 * PHI-functions are unknown.  They are of the form:
 *
 *                 h = PHI(h, h, ..., h)
 *
 * and are referred to as PHI-statements (also called
 * "expression-PHIs" in Chow's paper).  The method placePhis() inserts
 * these hypothetical PHI-functions into the CFG.
 *
 * Once the expression-PHIs have been placed, version numbers are
 * assigned to each hypothetical h.  Both real occurrences of the
 * expression and PHI-statements for the expression have a numbered h
 * value associated with them.  Occurrences that have the same version
 * number have identical values and any control flow path that includes
 * two difference h-versions must cross an assignment to an operand of
 * the expression or an PHI-statement for h.  The version numbers are
 * assigned in two passes over the CFG.  The first pass compiles of a
 * worklist containing all of the real occurrences that are defined by
 * an PHI-statement.  It also maintains a stack of real occurrences of
 * the expression.  The first pass optimistically assumes that versions
 * of the operands to a PHI-function are the same as the version of the
 * expression that is on top of the expression stack.  The second pass
 * corrects any false assumptions that were made during the first pass.
 * Since the first pass saw all of the occurrences, the versions of the
 * h-variables can be determined from the existence of a phi-function
 * (note lowercase phi) at that block.  In some cases, the occurrence
 * may be placed back into the worklist for further processing.  The
 * rename() method handles the assignment of version numbers using this
 * two-pass method.
 *
 * Now that the PHI-functions have been placed and version numbers have
 * been assigned, other information about the hypotheticals is extracted
 * and will be used to move redundent occurrences of the expression.
 * The first piece of information that is calculated is whether or not
 * an PHI-statement is "down-safe" (also referred to as being
 * "anticipated").  When version numbers were assigned to hypotheticals,
 * a use-def relationship was created: PHI-statements use operands that
 * are hypotheticals defined by other occurrences.  One can
 * conceptualize a directed graph consisting of PHI-statements and the
 * directed edges going from a use of a hypothetical to its definition.
 * This graph is referred to by Chow as the "SSA Graph".  (This is not
 * to be confused with the class SSAGraph which models the SSA Graph for
 * variables, not expressions.  This implementation does not directly
 * model the SSA Graph.)  Using the SSA Graph the down-safety of an
 * PHI-statement is computed by backwards propagation along the use-def
 * chains.  An PHI-statement is not down-safe if there is a control flow
 * path from that PHI-statement along which the expression is not
 * evaluated (i.e. there is no real occurrence) before the exit block is
 * encountered or one of the expression's variables is redefined.  Why
 * is down-safety important?  If an PHI-statement is not down-safe, it
 * is not worthwhile to hoist it any higher in the code.  If it were to
 * be hoisted, it might add a unnecessary evaluation of the expression
 * along some path.  This would break the golden rule of SSAPRE.  There
 * are two situations in which an PHI-statement is not down-safe.  The
 * first occurs when there is no path to exit along which the result
 * (left hand side) of the PHI-statement is not used.  The second occurs
 * when there is a path to exit along which the only use of the
 * PHI-statement result is as an operand to an PHI-statement which
 * itself is not down-safe.  The PHI-statement that fit the first
 * criterion can be marked as not down-safe during the renaming step.
 * In addition, each operand to an PHI-statement (called an PHI-operand)
 * is marked as "has real use" when the path to the PHI-operand crosses
 * a real occurrence of the same version of the expression.  Simply put,
 * an PHI-operand has a real use, if it is associated with a real
 * expression instead of an PHI-statement.  The down-safety of the
 * PHI-statements in the CFG is computed using of the downSafety() and
 * resetDownSafe() methods.
 *
 * The above description was written by Dave and Steve Lennon in early
 * September of 1998.  I'm surprised at how much we knew then.  Consult
 * the BLOAT Book for the conclusion to our exciting SSAPRE saga.
 */
...
public class SSAPRE {
   ...
   * @parsm context
   *        The EditorContext containing all the classes that BLOAT
   *         knows about.
   */
  ...
  /**
   * Performs partial redundency elimination on a given expression.
   * This method is called on every lexically-distinct expression in 
   * a method.
   *
  ...
  private void transform(ExprInfo exprInfo) {
    ...
    // for expressions, not variables.  However, the same Phi classes are used.
    placePhis(exprInfo);
    ...
    LocalVariable v = cfg.method().newLocal(type);
    VarExpr tmp = new LocalExpr(v.index(), type);
  ...
  private void collectOccurrences() {
	...
	if (beginTry.contains(block)) {
	  // If the block begins a protected region, then we must insert
	...
	expr.setKey(count.value++);
	worklist.addKill(expr.block(), 
			 new ExceptionKill(expr.key()));
      }
	...
	if (! NO_THREAD) {
	  ...
	  worklist.addKill(stmt.block(), 
			   new MemRefKill(stmt.key()));
	}
      }
  ...
  /** 
   * Returns a Set of Blocks that begin the protected regions in the
   * CFG.
   */
  private Set beginTry() {
    Set beginTry = new HashSet();
   ...
   *
   *    1) Place PHI at the expression's iterated dominance frontier (DF+)
   *    2) When there is a phi for a variable contained in the expression
   *       (this indicates an alteration in the expression)
   *
   * It is only necessary to insert a PHI at a merge point when the
   * expression will occur again after that block.
   */
  ...
  /**
   * Rename all occurrences of the expression.  placePhis went through
   * the CFG and placed PHI-functions at merge blocks in the code.  Now,
   * we have to go through and assign version numbers to all of the "h"
   * variables generated by the PHI-functions.
   * <p>
   * There are two methods outlined in [Chow 1997].  The first is more
   * straightforward (ya right, it took us two days to figure it out)
   * while the second is more space efficient.  The second method delays
   * the renaming of the "h" variables.  It makes two passes over the
   * CFG (actually, they are preorder traversals of the dominator tree).  
   * <p>
   ...
   * assume that versions of PHI operands are the same as the version on
   * top of the expression stack.  These assumptions are checked for 
   * correctness during the second pass.
   * <p>
   ...
   * first pass.  From the versions of the variables at the merge block
   * of a PHI, the versions of the variables at each predacessor block are
   * determined based on the presence or absence of a phi-function for the
   * at that merge block.  If the versions are different from the assumed
   * versions from the first pass, the operand is rest to null (bottom).
   * Otherwise, the operand is correct.  If the PHI operand is also defined
   * by a PHI, it is added to the worklost and is handled later.
   */
	...
	if (operand instanceof RealDef) {
	  ...
	  expr.visitChildren(new TreeVisitor() {
	    ...
	    public void visitLeaf(Expr expr) {
	      if (! leafIter.hasNext()) {
		// We've already examined all of the leaves, they
		// don't match
		match.value = false;
		return;
	      }
	      ...
	      // Factor the leaf through any var-phis there.  That is,
	      // If the leaf is defined by a phi-statement, use the
	      // corresponding phi-operand as the leaf.  If the leaves
	      // don't match (i.e. are not constants, variables, nor
	  ...
	  if (! match.value || leafIter.hasNext()) {
	    // If the leaves do not match (or if we didn't get to all
	...
	LEAVES:
	  while (leafIter.hasNext()) {
	    ...
	    if (leaf instanceof VarExpr) {
	      ...
	      if (leaf.parent() instanceof PhiJoinStmt) {
		PhiJoinStmt leafPhi = (PhiJoinStmt)
		  leaf.parent();
  ...
  /** 
   * This method performs the first pass of the delayed renaming
   * algorithm.  Recall that the original renaming algorithm kept a
   * stack for the "current" version number of each variable used in
   * the expression being renamed.  Well, when a real occurrence of
   * the expression is encountered, we don't need the stacks because
   * the real occurrence contains the current version numbers of the
   * variables.  So, we only need the version numbers when renaming
   * the "h" variables for PHI-operands.
   *
   * When this first pass encounters a PHI-operand, it optimistically
   * assumes that the version on top of the stack is the correct
   * version.  The "h" values for real occurrences will be handled
   * correctly.
   *
   * This implementation represents an occurrences "h" value by its
   * Def (the "setDef" method of ExprInfo).  This method performs a
   * pre-order traversal of the CFG's dominator tree and assigns
   * "names" (actually references to Defs) to occurrences of an
   * expression.
   *
   * The end result of this traversal is a worklist of real
   * occurrences that require further renaming.  Along the way, we
   * compute the down safety (or lack there of) of some
   * PHI-statements.
   *
   * @param block
  ...
  private void search(Block block, ExprInfo exprInfo, Expr top, Def topdef,
	...
	if (top == null && topdef instanceof Phi) {
	  // No real occurrence after the Phi, so the Phi is not down safe.
	  ((Phi) topdef).setDownSafe(false);
	}
  ...
  /** 
   * This method determines whether or not a given (real occurrence
   * of an) expression has the same operands as the target of a
   * PHI-statement.  That is, has one of the operands of the real
   * occurrence changed since the the PHI-statement?
   */
  ...
  private Expr buildPhiOperand(ExprInfo exprInfo,
			       final Phi phi, final Block pred) {
  ...
  /**
   * A Phi is not down safe if there is a control flow path from 
   * that Phi along which the expression is not evaluated before
   * exit or being altered by redefinition of one of the variables of the
   ...
   * Will be available determines where we end up placing an evaluation
   * of the expression.  
   *        Will be available = Can be available AND (not Later)
   */
   ...
   * of the expression".  If cba = 0, then the PHI-statement is "useless"
   * and uses of it are changed to tack (bottom).  Can be available depends
   * on the down safety of the PHI-statement.
   */
  ...
  /**
   * Resets the cba flag for a given PHI-expression and then iterates
   * over the PHI-statement's operands and resets them under certain
   * conditions.
   */
  private void resetCanBeAvail(ExprInfo exprInfo, Phi phi) {
	...
	if (operand == phi) {
	  ...
	  // If the use does not have a real use, set the use to tack (bottom).
	  other.setOperandAt(pred, null);

	  // Since we changed other (by setting one of its operands to
	  // tack), if other is not down safe, propagate this information
   ...
   * any later that this point without adding additional evaluation(s) 
   * along some path".  An expression is "interesting" when later is false.
   */
  private void computeLater(ExprInfo exprInfo) {
	...
	continue;
      ...
      //   1. is not tack
      //   2. has a real use
      // set later to false and propagate this information back up the CFG.
  ...
  private void resetLater(ExprInfo exprInfo, Phi phi) {
	...
	continue;
      ...
      // For PHI-statement that has the "h" defined by phi as an operand...
   ...
   * and evaluations of the expression.  It decides whether the results of
   * real occurrences should be computed on the spot (and saved to a 
   * temporary) or reloaded from a temporary.  Some PHI-statements are 
   * removed and some are replaced by PHI-statements operating on the
   * temporaries.  Additional evaluations of the expression may be added
   * where the expression is not available.
   */
   ...
   * @param top
   *        Top is used to determine when an element of Avail_def dominates
   *        a given occurrence.
   */
  ...
  private void codeMotion(ExprInfo exprInfo, VarExpr tmp,
	...
	if (exprInfo.save(real)) {
	  ...
	  Assert.isFalse(real.isDef(), "Can't reload a def: " +
			 real + " in " + real.parent());
  ...
  private void save(ExprInfo exprInfo, VarExpr tmp, Expr real,
		    SSAConstructionInfo consInfo) {
    if (DEBUG) {
      System.out.println("SAVE: " + real + " to " +
			 tmp + "--------------------------------");
    }
  ...
  private void reload(ExprInfo exprInfo, VarExpr tmp, Expr real,
		      SSAConstructionInfo consInfo) {
    if (DEBUG) {
      System.out.println("RELOAD: " + real + " to " +
			 tmp + "--------------------------------");
    }
  ...
  private void saveTarget(ExprInfo exprInfo, VarExpr tmp,
			  Expr real, SSAConstructionInfo consInfo) {
    if (DEBUG) {
      System.out.println("SAVE TARGET: " + real + " to " +
			 tmp + "--------------------------------");
    }
  ...
  /**
   * FirstOrderChecker is a TreeVistor that traverses an expression 
   * tree and determines whether or not it is first order.  A first
   * order expression 
   * Override all visitXXXExpr methods so that they do not visit children.
   * We only want to check the expr we first visit.
   */
  private final class FirstOrderChecker extends TreeVisitor {
    ...
    /**
     * A leaf in the expression tree consists of an expression that 
     * only references local variables, or an expression that is a
     * constant, or an expressions that stores into a local variable.
     */
    ...
    /**
     * An arithmetic expression is first-order if both its left and
     * right operands are leaves.
     */
    ...
    /**
     * An ArrayLengthExpr is first-order when the array whose length
     * is being taken is expressed as a leaf.
     */
    ...
    /**
     * An ArrayRefExpr is first order when both the array it references
     * and the index used to reference it are expressed as leaves.
     */
    ...
    /**
     * If a field is volatile (meaning that a field may be changed by
     * other threads), a reference to it is not first order.  I'm not too
     * sure why this makes any difference.
     */
	...
	try {
	  FieldEditor e = context.editField(expr.field());
    ...
    /**
     * Once again, an expression that references a volatile static field
     * is not first-order.
     *
     * @see #visitFieldExpr
     */
   ...
   * their values are changed inside methods.  When used as parameters
   * they must be declared as being final.
   */
  class Int {
    int value = 0;
  }
  ...
  /**
   * Phi represents a PHI-statement (PHI-function) for merging an 
   * expression along two paths.
   * <p>
   * Information about the operands to PHI-statements is maintained
   * in the PHI class.
   * <p>
   * A PHI-statement has the form:
   *      h = PHI(h, h)
   *
   * @see #operands
   */
  class Phi extends Def {
     ...
     * @param block
     *        The block in which this PHI-statement occurs.  Note that
     *        an PHI-statement can only occur in one block.  
     */
    ...
    /**
     * Sets the operands to a real occurrence of the expression.  Leaves
     * only apply to PHI-statements that are associated with a real
    ...
    public void setLeaves(List leaves) {
      if (DEBUG) {
	System.out.println("setting leaves of " + this +
			   " to " + leaves);
      }
     ...
     * PHI-statement.  It is assumed that this PHI-statement represents
     * a real occurrence.
     */
    public List leaves() {
    ...
    /**
     * Returns the operands of the PHI-statement.  This is just a list of
     * all of the block's predacessors.
     */
    ...
    /**
     * Sets an operand of this PHI-statement.  Recall that PHI-operands 
     * are associated with a predacessor block of the block in which the
    ...
    /**
     * Returns the PHI-operand of this PHI-statement associated with a
     * given block.  Recall that PHI-operands are associated with a
     * predacessor block of the block in which the PHI-statement resides.
     */
    ...
    /**
     * Determines whether or not a PHI-operand satisfies "insert".  For insert
     * to hold, the following conditions must be met:
     *   1. The PHI-statement is "will be available"
     *   2. The PHI-operand is tack (null), or "has real use" is false and
     *      the operand is defined by an PHI-statement that does not satisfy
     *      "will be available".
     *
     * @param block
    ...
    /**
     * Returns whether or not an PHI-statement satisfies "will be available".
     * "Will be available" is used to determine the locations in which to
     * insert evaluations of the expression in the finalize() pass.
     * <p>
    ...
    /**
     * Sets the "later" flag.  If the later flag is false, it means that
     * an evaluation of the expression may not be placed at any point below
    ...
    /**
     * Sets the "down-safe" flag.  An PHI-statement is "down-safe" if
     * there is no path from the PHI-statement to the exit block that
     * does not recalculate the expression.  If an PHI-statement is
     * "down-safe" it is worthwhile to attempt to hoist it up higher
     * in the program.
     * <p>
     * An PHI-statement is not "down-safe" when
     *   a) There is a path to exit along which the PHI-statement
     *      result is never used.
     *   b) There is a path to exit along which the only used of the
     *      result of the PHI-statement is an operand of an PHI-statement
     *      which itself is not "down-safe".
     */
   ...
   * on.  An occurrence of an expression can take one of three forms:
   *  1) A real occurrence of an expression (h = a+b)
   *  2) A (target of a) PHI function (h = PHI(...))
   *  3) An operand to a PHI function (PHI(h, ...))
   *
  ...
  private final class ExprInfo {
    ...
    private boolean[] realsSorted;   // Are the reals at a given block sorted?
     ...
     *
     * @param expr
     *        The expression (real occurrence) represented by this ExprInfo.
     * @param key
	...
	// Traverse the tree and determine whether expr accesses a final field.
    ...
    public void setReload(Expr expr, boolean flag) {
      if (DEBUG) {
	System.out.println("        setting reload for " +
			   expr + " to " + flag);
      }
    ...
    public void setSave(Expr expr, boolean flag) {
      if (DEBUG) {
	System.out.println("        setting save for " +
			   expr + " to " + flag);
      }
    ...
    public void setAvailDef(Def def, Def availDef) {
      if (DEBUG) {
	System.out.println("        setting avail def for " +
			   def + " to " + availDef);
      }
    ...
    public Def availDef(Def def) {
      ...
      if (DEBUG) {
	System.out.println("        avail def for " +
			   def + " is " + availDef);
      }
    ...
    /**
     * Sets the defining occurrence (the "h") of a given real
     * occurrence.
     */
    ...
    /**
     * Returns the Def (either a ReafDef or Phi) defining a given
     * occurrence of the expression modeled by this ExprInfo.
     */
    public Def def(Expr expr) {
      Def def = (Def) defs.get(expr);

      if (DEBUG) {
	System.out.println("        def for " +
			   expr + " is " + def);
      }
    ...
    /**
     * Notifies this ExprInfo of the existence of another real occurrence
     * of the expression.
     */
    ...
    /**
     * Notifies this ExprInfo of the existence of an PHI-statement for
     * this expression.  If the PHI is not already present, a new
     * Phi instance is created for it and placed in the phis array.
     *
     * @param block
	...
	if (DEBUG) {
	  System.out.println("    add phi for " + prototype +
			     " at " + block);
	}
    ...
    /**
     * Returns the PHI occurrence for this expression at a given Block
     * in the code.
     */
    ...
    /**
     * Returns the real occurrences of this expression at a given Block
     * in the code.
     */
    ...
    /**
     * Returns a List of the real occurrences of the expression and
     * any Kill expressions contained in a given Block in the code.
     */
     ...
     *
     * Recall that the key of each occurrence node was set to its 
     * preorder number in collectOccurrences.
     */
    ...
    /**
     * Sorts a lists of Kills into preorder.  That is, the Kills in a
     given block are sorted by the pre-order number.
     */
   ...
   * containing all of the first-order expressions in the CFG (method).
   * The worklist is assembled in collectOccurrences() and its expressions
   * are used throughout SSAPRE.
   *
   * @see #collectOccurrences
   */
  class ExprWorklist {
     ...
     * If necessary, an ExprInfo is created to represent the expression.
     */
    public void addReal(Expr real) {
      if (DEBUG) {
    ...
    public void addKill(Block block, Kill kill) {
      if (DEBUG) {
	System.out.println("    add alias to worklist=" +
			   kill.expr + " " + kill);
      }
   ...
   * (monitorenter and monitorexit) block of code, or an expression
   * that accesses a memory location (MemRefExpr) and defines a variable,
   * or an expression that invokes a method is encountered.
   *
   * @see #collectOccurrences
   */
  ...
  class ExprKey {
	...
	while (iter1.hasNext() && iter2.hasNext()) {
	  ...
	  if (child1 instanceof StackExpr !=
	      child2 instanceof StackExpr) {
	    return false;
	  }
	  ...
	  if (child1 instanceof VarExpr &&
	    ...
	    if (phiRelatedFind(child1.def()) !=
		phiRelatedFind(child2.def())) {
	    ...
	    Assert.isTrue(child1 instanceof ConstantExpr ||
			  child2 instanceof ConstantExpr,
			  "neither " + child1 + " nor " + child2 +
			  " are constants");
