	...
	static final class ProspectiveTagTypeIterator implements Iterator<TagType> {
		...
		public ProspectiveTagTypeIterator(final Source source, final int pos) {
			...
			try {
				...
				while (true) { // cursor.children is an array of TagTypeRegister!
					final TagTypeRegister child=cursor.getChild(parseText.charAt(pos+(posIndex++))); // can generate constraints!
					if (child==null) break;
					cursor=child;
				}
