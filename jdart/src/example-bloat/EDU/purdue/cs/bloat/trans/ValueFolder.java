 ...
 * <tt>Node</tt> to determine whether or not it can be simplified.
 * For instance, redundent checks are removed and algebraic identities
 * are exploited.
 *
...
class ValueFolder extends TreeVisitor {
  ...
  public void visitCastExpr(CastExpr expr) {
	...
	expr.castType().equals(Type.STRING)) {
  ...
  public void visitIfCmpStmt(IfCmpStmt stmt) {
	...
	if (replace) {
	  JumpStmt jump = new IfZeroStmt(cmp,
					 (Expr) stmt.right().clone(),
					 stmt.trueTarget(), 
					 stmt.falseTarget());
	  jump.catchTargets().addAll(stmt.catchTargets());
	  node = jump;
