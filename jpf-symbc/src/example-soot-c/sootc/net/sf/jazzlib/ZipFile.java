...
//    System.out.println(b[0] + ":" + b[1] + ":" + b[2] + ":" + b[3]);
  ...
  private void readEntries() throws ZipException, IOException
  {
	...
	raf.seek(pos--);
	ret = readLeInt(raf, ebs) ;
      }
    while (ret != ENDSIG);

//	System.out.println("result is : " + ret + " ENDSIG is : " + ENDSIG);
...
//    	System.out.println(i);    	
  ...
  private ZipEntry readEntry(String entryname) throws ZipException, IOException
  {
    /* Search for the End Of Central Directory.  When a zip comment is 
     * present the directory may start earlier.
     * FIXME: This searches the whole file in a very slow manner if the
...
//    System.out.println(count);
    ...
    {    	
    	...
    	entry.offset = offset;
    	entries.put(name, entry);
    	//System.out.println(i + ":" + name + ":" + entryname);
    	if (name.equals(entryname)) return entry;
    }
  ...
  private ZipEntry getEntries(String name) throws IOException
  {
	  synchronized(raf)
	  {
		  if (closed)
...
//    ZipEntry zipEntry = (ZipEntry) entries.get(name);
    ZipEntry zipEntry = getEntries(name);
