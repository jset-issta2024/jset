...
import ca.mcgill.sable.sablecc.lexer.Lexer;
import ca.mcgill.sable.sablecc.lexer.LexerException;
import ca.mcgill.sable.sablecc.node.Start;
import ca.mcgill.sable.sablecc.parser.Parser;
import ca.mcgill.sable.sablecc.parser.ParserException;
...
// $Format: "        System.out.println(\"SableCC version $ProjectVersion$\");"$
    ...
    {
    	arguments = new String[]{"jdart/src/example-sablecc/simple.scc"};
    ...
    public static void start(char[] data) throws FileNotFoundException, ParserException, LexerException, IOException {
    	try{
	    	FileReader temp;
	    	
	        // Build the AST
//	        Start tree = new Parser(new Lexer(new BufferedReader(temp = new FileReader(in)))).parse();
            Start tree = new Parser(new Lexer(new CharArrayReader(data))).parse();
