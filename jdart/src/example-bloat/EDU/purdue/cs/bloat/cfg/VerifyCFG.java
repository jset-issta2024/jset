 ...
 * that certain properties of the graph are true.  For instance, value
 * numbers of expressions are not equal to -1, node connections are
 * consistent, exception handlers are set up correctly, etc.  Mostly
...
public class VerifyCFG extends TreeVisitor {
  ...
  /** 
   * Constructor.  Since value numbers are not strictly part of the
   * control flow graph, they may or may not be checked.  For
   * instance, if a CFG is being verfied before value numbers are
   * assigned, we would not want to check them.
   *
   ...
   * Examine the uses of a variable that is defined in the CFG.  Make
   * that all uses are reachable (i.e. are in the CFG).
   */
  public void visitFlowGraph(FlowGraph cfg) {
    ...
    while (e.hasNext()) {
      ...
      Assert.isTrue(nodes.contains(use),
		    "use = " + use + " (" + System.identityHashCode(use) +
		    ") is not in the CFG");
    }
  ...
  /** 
   * First make sure that the <tt>Block</tt> indeed is in the CFG.
   * If the block begins an exception handler, then make sure that all
   * edges from protected blocks lead to the handler block.  Also make
   * sure that all of the handler block's predacessor lead to
   * protected blocks.  Finally, make sure that the
   * successor/predacessor relationship holds.  
   */
  public void visitBlock(Block block) {
    ...
    if (handler != null) {
      HashSet handlerPreds = new HashSet();
	...
	handlerPreds.addAll(cfg.preds(prot));
      ...
      HashSet extra = new HashSet(cfg.preds(block));
    ...
    while (e.hasNext()) {
      ...
      Assert.isTrue(cfg.succs(pred).contains(block),
		    pred + " has no succ " + block);
      Assert.isTrue(cfg.preds(block).contains(pred),
		    block + " has no pred " + pred);
    }
    ...
    while (e.hasNext()) {
      ...
      Assert.isTrue(cfg.succs(block).contains(succ),
		    block + " has no succ " + succ);
      Assert.isTrue(cfg.preds(succ).contains(block),
		    succ + " has no pred " + block);
    }
  ...
  /**
   * Make sure that all of the targets of the <tt>jsr</tt> are valid.
   * The only target is the entry block of the subroutine.
   */
   ...
   * Targets consist of the true target, the false target, and the
   * first blocks of any exceptions that may be thrown by the if
   * statement.  
   */
  ...
  /** 
   * Verifies information about the targets of a jump in a given
   * block.  First, make sure that the number of targets is the same
   * as the number of sucessor nodes to the block.  Make sure that
   * targets of all of the jumps are indeed in the CFG.  Make sure
   * that every target is a successor of the block.  
   */
  private void verifyTargets(Block block, Set targets)
  {
    Assert.isTrue(targets.size() == cfg.succs(block).size(),
		  block + " has succs " + cfg.succs(block) +
		  " != " + targets + " in " + block.tree().lastStmt());

    Iterator iter = targets.iterator();

    while (iter.hasNext()) {
      Block target = (Block) iter.next();
      Assert.isTrue(block.graph().hasNode(target),
		    target + " is not in the CFG");
      Assert.isTrue(cfg.succs(block).contains(target),
		    target + " is not a succ of " + block + " " +
		    block.tree().lastStmt());
    }
  }

  /** 
   * If desired, makes sure that the store expression's value number
   * is not -1.  Makes sure that the store expression's block and
   * parent <tt>Node</tt> are what we expect them to be.  If the type
   * of the <tt>StoreExpr</tt> is void, then make sure that its parent
   * is an <tt>ExprStmt</tt> (i.e. make sure it is not nested within
   * another expression).  
   */
  public void visitStoreExpr(StoreExpr node)
  {
    ...
    Assert.isTrue(node.block() == block,
    ...
    if (node.type().isVoid()) {
      Assert.isTrue(parent instanceof ExprStmt,
  ...
  /** 
   * Make sure that the <tt>Node</tt> resides in the block that we
   * expect it to and that it has the expected parent expression tree
   * <tt>Node</tt>.  Make sure that the children of this <tt>Node</tt>
   * are also correct.
   */
  public void visitNode(Node node)
  {
    ...
    Assert.isTrue(node.block() == block,
		  node + ".block() = " + node.block() + " != block = " + block);
    Assert.isTrue(node.parent() == parent,
  ...
  /**
   * If desired, make sure that the value number of the <tt>Expr</tt>
   * is not -1.
   */
  public void visitExpr(Expr expr)
  {
    if (checkValueNumbers) {
      Assert.isTrue(expr.valueNumber() != -1,
		    expr + ".valueNumber() = -1");
    }
  ...
  /** 
   * Make sure that the <tt>VarExpr</tt> either defines a local
   * variable, is defined by another expression, or is the child of a
   * <tt>PhiStmt</tt> (therefore making the <tt>VarExpr</tt> a
   * phi-variable).  
   */
