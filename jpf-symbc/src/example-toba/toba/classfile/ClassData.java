...
public ClassFile d = null;

public
ClassData(ClassFile d1)
    {
		this.d = d1;
    }
...
//    	d.close();
    ...
    // read constant table
    constants = Constant.load(d); // generate many constraints!!!

    // read access flags, class, superclass
    access = d.readUnsignedShort();
    /*bug, fixed by zhenbang */
    int t = d.readUnsignedShort();
    if (t < constants.length && t >= 0 && constants[t] != null && constants[t].value instanceof ClassRef)
    	name = ((ClassRef)constants[t].value).name;
    else
    	name = "";
    
//    if (major == 111) //bug injected
//    	d.close();
//    
    cname = Names.hashclass(name);
    fname = Names.classfile(name);
...
//	supername = ((ClassRef)constants[i].value).name;
	...
	if (j > 0 && j < constants.length && constants[j].value instanceof ClassRef) // bug fix by zhenbang
	...
	if (f.name != null) f.cname = Names.hashvar(f.name);
        f.classfilePos = i;
	fields[i] = f;
    }
    addSymFields (fields);
    ...
    IOException
{
    ...
    ncd = new ClassData (d);
    ncd.load(); // all the constraints generated in this functions!
