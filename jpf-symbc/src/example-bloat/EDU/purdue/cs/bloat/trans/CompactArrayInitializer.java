...
/**
 * <tt>CompactArrayInitializer</tt> optimizes the initialization of 
 * arrays by transforming the initialization code into a loop that
 * loads the array elements from a string in the class's constant pool.
 */
public class CompactArrayInitializer implements Opcode
{
  ...
  private static final String[] STATES = {
    ...
    "EXPECT_NEW",
    "EXPECT_DUP",
    "EXPECT_INDEX_OR_SIZE",
    "EXPECT_VALUE_OR_SIZE_OR_NEW",
    "EXPECT_STORE_OR_NEW",
    "EXPECT_PUT_OR_DUP" };
  ...
  static {
    ...
    GET_CHARS = new MemberRef(Type.STRING,
			      new NameAndType("getChars", 
					      Type.getType("(II[CI)V")));
  }
  
  /**
   * Some Java compilers initialize arrays using straight-line code.  For
   * classes that have large, initialized arrays this results in 
   * unnecessarily large classfiles.  <tt>CompactArrayInitializer</tt>
   * examines a method (via its <tt>MethodEditor</tt>) creates a string
   * in the method's class's constant pool that contains all of the 
   * elements of the initialized array.  After the old initialization 
   * code is removed from the method, new code is inserted that 
   * essentially is a loop that loads each element from the string into
   * the array.  Note that only arrays of <tt>int</tt>, <tt>short</tt>,
   * <tt>char</tt>, <tt>byte</tt>, and <tt>boolean</tt> are compacted.
   *
   * @param method
  ...
  public static boolean transform(MethodEditor method)
  {
    ...
    boolean filled = false;       // Was the constant string generated and entered?
    
    int state = EXPECT_SIZE;      // The state that we are currently in
	...
	switch (state) 
	  ...
	  case EXPECT_INDEX_OR_SIZE:
		...
		if (index < data.length) {
		  state = EXPECT_VALUE_OR_SIZE_OR_NEW;
		}
		else {
	  ...
	  case EXPECT_PUT_OR_DUP:
		...
		if (data.length >= THRESHOLD) {
		  fillArray(method, buf, elementType, data);
		  filled = true;
		}
		state = EXPECT_SIZE;
		break;
  ...
  /**
   * Construct a UTF8 string that stores the contents of an integral
   * (int, char, or byte/boolean) array.  Each element of the UTF8 
   * string is 16 bits wide meaning that an int is stored into two
   * elements, a char is store in one element, and two bytes/booleans
   * are stored in one element.  Essentially each integer in the
   * <tt>data</tt> parmeter is converted into a character and placed
   * in an array.
   * <p>
   * A UTF8 string cannot be larger than 64K.  To be on the safe side,
   * we do not generate UTF8 strings that are larger than 32K.
   * <p>
   * We then remove all of the old initialization code, so that the
   * previous instruction is a <tt>newarray</tt> opcode (meaning that
   * the array object will be on top of the stack).
   * <p>
   * Then new code is generated for loading data from the UTF8 string.
   * Loading character data is relatively straightforward.  The 
   * <tt>getChars</tt> method is invoked on the UTF8 string (an instance
   * of <tt>String</tt>) to copy characters from the string into a 
   * character array.
   * <p>
   * A little more work has to be done for non-character data types.  
   * First, the UTF8 string is read into a (local) character array.  
   * Then, the data is extracted from the character array and placed in
   * the new array.  For some data types, labels are added to the 
   * program.
   *
   * @param m
   ...
   *        
   */
  private static void fillArray(MethodEditor m,	ArrayList2 buf, 
				Type elementType, int[] data)
  {
	...
	// We've reached the limit on the size of the constant pool string.
    ...
    if (sb.length() > 0) {
      // If we've got leftovers, add it to the end of the current image entry.
      image.add(sb.toString());
    ...
    for (int i = buf.size()-1; i >= 0; i--) {
      Instruction inst = (Instruction) buf.get(i);
      if (inst.opcodeClass() == opcx_newarray) {
    ...
    if (elementType.equals(Type.CHARACTER)) {
      ...
      LocalVariable array = m.newLocal(Type.OBJECT);  // character array
      
      buf.add(new Instruction(opcx_dup, array));
      buf.add(new Instruction(opcx_astore, array));
      
      // Call getChars() for every image

      for (int i = 0; i < image.size(); i++) {
	String im = (String) image.get(i);
	Integer start = (Integer) startIndex.get(i);
	Integer end = (Integer) endIndex.get(i);
	
	// void getChars(int srcBegin, int srcEnd, char dst[], 
	//               int dstBegin)

	buf.add(new Instruction(opcx_ldc, im));       // String
	buf.add(new Instruction(opcx_ldc, new Integer(0)));
	buf.add(new Instruction(opcx_ldc, new Integer(im.length())));
	buf.add(new Instruction(opcx_aload, array));
	buf.add(new Instruction(opcx_ldc, start));    // dstBegin
	buf.add(new Instruction(opcx_invokevirtual, GET_CHARS));
      }
      ...
      LocalVariable array = m.newLocal(Type.OBJECT);    // char array
	...
	buf.add(new Instruction(opcx_ldc, start));
	buf.add(new Instruction(opcx_istore, index1));
	
	buf.add(new Instruction(opcx_ldc, new Integer(0)));
	buf.add(new Instruction(opcx_istore, index2));
	
	// Create a new array of characters and copy the UTF8 string
	// into it.
	buf.add(new Instruction(opcx_ldc, new Integer(im.length())));
	buf.add(new Instruction(opcx_newarray, Type.CHARACTER));
	buf.add(new Instruction(opcx_astore, array));
	buf.add(new Instruction(opcx_ldc, im));
	buf.add(new Instruction(opcx_ldc, new Integer(0)));
	buf.add(new Instruction(opcx_ldc, new Integer(im.length())));
	buf.add(new Instruction(opcx_aload, array));
	buf.add(new Instruction(opcx_ldc, new Integer(0)));
	...
	if (elementType.equals(Type.SHORT)) {
	  // Load an integer from the character array and then subtract
	  // 32768 (0x8000) from it.  Convert the integer to a short 
	  // and store it in the destination array (which happens to be
	  // on the top of the stack).

	  buf.add(new Instruction(opcx_dup));
	  buf.add(new Instruction(opcx_iload, index1));
	  buf.add(new Instruction(opcx_iinc,
				  new IncOperand(index1, 1)));
	  buf.add(new Instruction(opcx_aload, array));
	  buf.add(new Instruction(opcx_iload, index2));
	  buf.add(new Instruction(opcx_iinc,
				  new IncOperand(index2, 1)));
	  buf.add(new Instruction(opcx_caload));
	  buf.add(new Instruction(opcx_ldc, new Integer(32768)));
	  buf.add(new Instruction(opcx_isub));
	  buf.add(new Instruction(opcx_i2s));
	  buf.add(new Instruction(opcx_sastore));
	  ...
	  buf.add(new Instruction(opcx_iinc,
				  new IncOperand(index2, 1)));
	  buf.add(new Instruction(opcx_caload));
	  buf.add(new Instruction(opcx_istore, tmp));
	  ...
	  buf.add(new Instruction(opcx_iinc,
				  new IncOperand(index1, 1)));
	  buf.add(new Instruction(opcx_iload, tmp));
	  buf.add(new Instruction(opcx_ldc, new Integer(8)));
	  buf.add(new Instruction(opcx_iushr));
	  buf.add(new Instruction(opcx_i2b));
	  buf.add(new Instruction(opcx_bastore));
	  
	  // If we've read the last byte, go home
	  // if (i >= end) break
	  buf.add(new Instruction(opcx_iload, index1));
	  buf.add(new Instruction(opcx_ldc, end));
	  buf.add(new Instruction(opcx_if_icmpge, bottom));
	  ...
	  buf.add(new Instruction(opcx_iinc,
				  new IncOperand(index1, 1)));
	  buf.add(new Instruction(opcx_iload, tmp));
	  buf.add(new Instruction(opcx_ldc, new Integer(0xff)));
	  buf.add(new Instruction(opcx_iand));
	  buf.add(new Instruction(opcx_i2b));
	  buf.add(new Instruction(opcx_bastore));
	  ...
	  buf.add(new Instruction(opcx_ldc, new Integer(16)));
	  buf.add(new Instruction(opcx_ishl));
	  ...
	  // Or the higher-order bits and the lower-order bits together
	  // and store the result in the integer array.
	  buf.add(new Instruction(opcx_ior));
	  buf.add(new Instruction(opcx_iastore));
	}
	...
	buf.add(new Instruction(opcx_iload, index1));
	buf.add(new Instruction(opcx_ldc, end));
	buf.add(new Instruction(opcx_if_icmplt, top));
...
/**
 * Recall that Nate used beta versions of the JDK1.2 util classes to
 * build BLOAT.  While most of the conversion to the final util API was
 * simple, there were a couple of changes made that force us to make
 * some changes.
 *
 * The final version of the API makes the ArrayList.removeRange() 
 * method protected.  So, we have to make this silly wrapper class in
 * order to access it.  Silly.
 */
