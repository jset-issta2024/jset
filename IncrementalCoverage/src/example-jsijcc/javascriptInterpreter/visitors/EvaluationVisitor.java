...
import static javascriptInterpreter.tree.JavascriptTreeConstants.JJTLEFTSIDEEXPRESSION;
import static javascriptInterpreter.visitors.JSToJavaUtils.countCommas;
import static javascriptInterpreter.visitors.JSToJavaUtils.doubleToBoolean;
...
public class EvaluationVisitor extends JavascriptDefaultVisitor {
    ...
    public JavascriptType visit(ASTupdateExpression node, Context data){
        ...
        if(node.jjtGetFirstToken() != operand.jjtGetFirstToken() || node.jjtGetLastToken() != operand.jjtGetLastToken()){
            ...
            //if (variableReference == null) return null; /* bug fixed by zhenbang */
    ...
    public JavascriptType visit(ASTunaryExpression node, Context data){
        ...
        if(node.jjtGetFirstToken() != operand.jjtGetFirstToken()){// && operand.jjtAccept(this, data) != null /*fixed by zhenbang*/){
    ...
    public JavascriptType visit(ASTassignmentExpression node, Context data){
        ...
        if(node.jjtGetChild(0).getId() == JJTLEFTSIDEEXPRESSION){
            ...
            //assignment
            switch(operator){
                case "=" : if(variableReference.getIdentifierName() == null) {// && /*fixed by zhenbang*/ variableReference.getIdentifierName() == null){
                    return assignedValue;
                }
                else {//if (variableReference != null /*fixed by zhenbang*/){
