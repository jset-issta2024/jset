...
final class StartTagTypeUnregistered extends StartTagType {
	...
	protected Tag constructTagAt(final Source source, final int pos) {
		...
		//modified by zhenbang
		//if (source.logger != null && source.logger.isErrorEnabled()) source.logger.error(source.getRowColumnVector(tag.getBegin()).appendTo(new StringBuilder(200).append("Encountered possible StartTag at ")).append(" whose content does not match a registered StartTagType").toString());
		return tag;
	}
}
