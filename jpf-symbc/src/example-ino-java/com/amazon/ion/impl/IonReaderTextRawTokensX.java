...
//        System.out.print("current char: "+(char)c+"  ");
        return c;
    }
...
//        System.out.print("Current pos: "+_stream._pos+"   ");
//        System.out.println("Current token: "+_token);
        
        if(_token==0) //unsymb TOKEN_EOF
        	return _token;
        
        if(InojavaConfig.SYMB_FLAG==false || InojavaConfig.TOKEN_SYMB==false){
            return _token;
      	}
        int c;
		if (oldtoken == _token && oldpos == _stream._pos) {
//			c = Debug.makeConcolicInteger("sym_token_" + (token_index - 1), "" + (int) token , "1");
			c = Debug.makeSymbolicInteger("" + (int) token );
		} else {
//			System.out.println("start to create: " + pos + ":" + token + ":" +token_index);
			c = Debug.makeSymbolicInteger("" + (int) _token );
			token_index++;
			oldtoken = _token;
			oldpos = _stream._pos;
		}
