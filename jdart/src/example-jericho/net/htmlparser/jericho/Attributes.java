...
public final class Attributes extends SequentialListSegment<Attribute> {
	...
	private static Attributes construct(final Source source, final String logType, ParsingState parsingState, final int logBegin, int attributesBegin, final int maxEnd, final StartTagType startTagType, final String tagName, final int maxErrorCount) {
		...
		if (tagName!=null) {
			...
			if (attributesBegin==-1) attributesBegin=logBegin+1+tagName.length();// can define the end!
		...
		try {
			while (!isTerminatingCharacter) {
				if (i==maxEnd || startTagType.atEndOfAttributes(source,i,isClosingSlashIgnored)) isTerminatingCharacter=true; // can generate constraints!
				final char ch=parseText.charAt(i);
				// First check if there is a server tag in this position:
				if (ch=='<') { // can generate constraints!
				...
				switch (parsingState) {
					case IN_VALUE:
							...
							if (quote==' ') {
								...
								if (isTerminatingCharacter) {
									if (i==maxEnd) {
										...
										if (source.logger != null && source.logger.isErrorEnabled()) log(source,logType,tagName,logBegin,"terminated in the middle of a quoted attribute value",i);
						...
						break;
					case IN_NAME:
					//	System.out.println("BBBBBBBBBBBBBBB2");
						...
						break;
					case AFTER_NAME:
						//System.out.println("BBBBBBBBBBBBBBB3");
					...
					case BETWEEN_ATTRIBUTES:
						//System.out.println("BBBBBBBBBBBBBBB4");
						if (!isTerminatingCharacter) {
						...
						break;
					case START_VALUE:
					//	System.out.println("BBBBBBBBBBBBBBB5");
						currentBegin=i;
						if (isTerminatingCharacter) {
						...
						break;
					case AFTER_TAG_NAME:
						//System.out.println("BBBBBBBBBBBBBBB6");
						// cannot generate constraints in this case!
						if (!isTerminatingCharacter) {
							if (!isWhiteSpace(ch)) {
								if (isInvalidEmptyElementTag(startTagType,source,i,logType,tagName,logBegin)) break;
