 ...
 * </pre>
 * <p>
 * I made modifications to the algorithm to compute FUD chains and to run
 * the algorithm separately for each variable similar to the SSAPRE algorithm.
...
public class SSA {
   ...
   * (SSA) form.  First, the CFG is traversed and a list of all
   * variables (both local and stack) eligible for SSA renaming is
   * compiled.  Variables are represented by instances of
   * <tt>SSAConstructionInfo</tt>.  Each of these variables is then
   * transformed.
   *
   * @see #transform
  ...
  /** 
   * Performs the actions necessary to convert a CFG into SSA form
   * with respect to one variable.  The variable's information is
   * stored in the <tt>SSAConstructionInfo</tt>.
   */
   ...
   * <tt>SSAConstructionInfo</tt> objects for each variable in the
   * CFG.  Returns the <tt>SSAConstructionInfo</tt>s for the variables
   * in the CFG.
   */
  private static Iterator collectVars(final FlowGraph cfg) {
	...
	SSAConstructionInfo info = (SSAConstructionInfo)
	  infos.get(key);
   ...
   * implementation only places phi functions for variables that are
   * live on entry to at least one block.  That is, if a variable is only
   * used within one block, we don't bother searching for a place to put
   * phi functions for it.
   *
   * @param cfg
   ...
   */
  private static void placePhiFunctions(FlowGraph cfg,
   ...
   * <tt>PhiCatchStmt</tt> for the variable in question in the handler
   * of the exception thrown by the protected region (meaning that the
   * variable is used in the protected region), the variable becomes
   * an operand to the <tt>PhiCatchStmt</tt>.
   *
   * @param info
   *        The variable (LocalExpr) that we're dealing with
   * @param block
   *        The block in a potentially protected region.  If the block
   *        is indeed in a protected region, the occurrence of the 
   *        the variable represented by info becomes an operand to
   *        the PhiCatchStmt at the beginning of the protected
   *        region's handler.
   * @param def
   *        The defining occurrence of the variable stored in info.  */
  private static void addCatchPhiOperands(SSAConstructionInfo info,
					  Block block, LocalExpr def) {
	...
	PhiCatchStmt phi = (PhiCatchStmt)
	  info.phiAtBlock(handler.catchBlock());
  ...
  /** 
   * The actual renamining is done by the search method.  This method
   * just takes care of <Tt>PhiReturnStmts</tt>.
   */
  private static void rename(FlowGraph cfg, SSAConstructionInfo info) {
	...
	PhiJoinStmt entry = (PhiJoinStmt)
	  info.phiAtBlock(sub.entry());
	...
	while (paths.hasNext()) {
	  ...
	  PhiReturnStmt ret = (PhiReturnStmt)
	    info.phiAtBlock(path[1]);

	  if (ret != null) {
	    DefExpr def = ret.operand().def();

	    if (def != entry.target()) {
	      // If the operand of the PhiReturnStmt is different from
	      // the new SSA variable defined by the PhiCatchStmt at
	      // the beginning of the subroutine, then the variable
	      // was defined in the subroutine, so the operand to the
	      // PhiReturnStmt is the correct SSA variable.  This is
	      // like the variable "b" in figure 3.5 in Nate's Thesis.
	      continue;
	    }
  ...
  /**  
   * Does the actual renaming.  It keeps track of the most recent
   * occurrence of an (SSA numbered) variable and recalculates the
   * definitions of variables appropriately.
   *
   * @param info
   *        SSAConstructionInfo representing the variable being
   *        converted into SSA form.
   * @param top
   *        "Top" of the variable stack for the variable in question.
   *        Each variable has a "stack" associated with it.  The top
   *        of the stack contains the current SSA name of the
   *        variable. It can also be thought of as the "most recent
   *        definition" of the variable.
   * @param block
   *        Basic block in which the variable is being renamed.
   */
  private static void search(FlowGraph cfg, SSAConstructionInfo info, 
                             VarExpr top, Block block) {
    if (DEBUG) {
      System.out.println("  renaming " + info.prototype +
			 " in " + block);
    }
   ...
   * associated with that block.  Up until this point, the phi statement
   * is only maintained in SSAConstructionInfo.  Note that the phi statement
