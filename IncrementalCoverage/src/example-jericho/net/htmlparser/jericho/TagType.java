...
public abstract class TagType {
	...
	static final Tag getTagAt(final Source source, final int pos, final boolean serverTagOnly, final boolean assumeNoNestedTags) {
		//System.out.println("now pos+"+pos);
		final TagTypeRegister.ProspectiveTagTypeIterator prospectiveTagTypeIterator=new TagTypeRegister.ProspectiveTagTypeIterator(source,pos);//can generate constraints!
		// prospectiveTagTypeIterator is empty if pos is out of range.
		while (prospectiveTagTypeIterator.hasNext()) {
		//	System.out.println("iiiiiiiiiiiiiiiiiiiiiiiiiiii2+"+pos);
			...
			try {
				final Tag tag=tagType.constructTagAt(source,pos); // generate constraints!
