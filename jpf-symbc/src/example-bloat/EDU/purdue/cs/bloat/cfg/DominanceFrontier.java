...
/** <tt>DominanceFrontier</tt> is used to calculate the <i>dominance
 * frontier</i> of each node in a control flow graph.  
 * <p>
 * The <i>dominance frontier</i> of a node x is the set of all nodes
 * w such that x dominates a predacessor of w, but does not strictly
 * dominate w.  Basically, nodes in the dominance frontier have one
 * parent that <b>is</b> dominated by x and at least one parent that
 * <b>is not</b> dominated by x.
 * <p>
 * <tt>DominanceFrontier</tt> can be used to calculate both the
 * dominance (forward) and the postdominance (reverse) frontiers for
...
{
  /**
   * Calculates the dominance frontier for a cfg and notifies the blocks
   * in it appropriately.
   *
   * @param graph
  ...
  /** 
   * Recursively traverses the cfg and builds up the dominance
   * frontier. 
   * <p> 
   * A block n's dominance frontier is the union of two sets of nodes.
   * The first set is the nodes in the dominance frontier of the nodes
   * that n dominates that are not dominated by n's immediate 
   * dominator.  The second set consists of the successors of n that
   * are not strictly dominated by n.
   *
   * @param block
   ...
   *
   * @return
   *        The blocks in the (post)dominance frontier of block */
  private static LinkedList calcFrontier(Block block,
					 FlowGraph graph, boolean reverse)
  {
    ...
    Iterator succs = reverse
      ? graph.preds(block).iterator()
